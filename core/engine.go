...
func (e *Engine) handleWorkspaceCommand(p Platform, msg *Message, args []string) {
	...
	case "":
		if supportsCards(p) {
			e.replyWithCard(p, msg.ReplyCtx, e.renderWorkspacePickerCardSafe(msg.SessionKey, 1))
			return
		}
...
// workspacePickerPageSize is the max workspace rows per card page.
const workspacePickerPageSize = 20
...
type workspacePickerEntry struct {
	Name       string
	Path       string
	ActivityAt time.Time
}

func parseWorkspaceBindActionArgs(args string) (string, int) {
	page := 1
	if args == "" {
		return "", page
	}
...
func (e *Engine) listWorkspacePickerEntries() ([]workspacePickerEntry, error) {
	entries, err := os.ReadDir(e.baseDir)
	if err != nil {
		return nil, err
	}
	states := map[string]*workspaceState(nil)
	if e.workspacePool != nil {
		states = e.workspacePool.All()
	}
	items := make([]workspacePickerEntry, 0, len(entries))
	for _, entry := range entries {
		...
		path := normalizeWorkspacePath(filepath.Join(e.baseDir, name))
		info, err := entry.Info()
		if err != nil {
			return nil, err
		}
		...
		if state := states[path]; state != nil {
			if last := state.LastActivity(); !last.IsZero() {
				activityAt = last
			}
		}
		items = append(items, workspacePickerEntry{
			Name:       name,
			Path:       path,
...
func workspacePickerPageBounds(page, total int) (start, end, totalPages int) {
	if total <= 0 {
		return 0, 0, 1
	}
	if total <= 10 {
	...
	totalPages = 1 + (total-10+workspacePickerPageSize-1)/workspacePickerPageSize
	if page < 1 {
		page = 1
	}
	if page > totalPages {
	...
	if page == 1 {
		...
		return 0, end, totalPages
	}
	start = 10 + (page-2)*workspacePickerPageSize
	end = start + workspacePickerPageSize
	if end > total {
		end = total
	}
...
func (e *Engine) renderWorkspacePickerCardSafe(sessionKey string, page int) *Card {
	card, err := e.renderWorkspacePickerCard(sessionKey, page)
	if err != nil {
		return e.simpleCard(e.i18n.T(MsgCardTitleWorkspace), "red", err.Error())
	}
	return card
}

func (e *Engine) renderWorkspacePickerCard(sessionKey string, page int) (*Card, error) {
	entries, err := e.listWorkspacePickerEntries()
	if err != nil {
		return nil, fmt.Errorf(e.i18n.T(MsgWsResolutionError), err)
	}

	channelKey := extractWorkspaceChannelKey(sessionKey)
	binding, bindingKey, usable := e.lookupEffectiveWorkspaceBinding(channelKey)
	currentWorkspace := ""
	if usable && binding != nil {
		currentWorkspace = normalizeWorkspacePath(binding.Workspace)
	}

	cb := NewCard().Title(e.i18n.T(MsgCardTitleWorkspace), "turquoise")
	switch {
	case usable && binding != nil && bindingKey == sharedWorkspaceBindingsKey:
		cb.Markdown(e.i18n.Tf(MsgWsInfoShared, binding.Workspace, binding.BoundAt.Format(time.RFC3339)))
	case usable && binding != nil:
		cb.Markdown(e.i18n.Tf(MsgWsInfo, binding.Workspace, binding.BoundAt.Format(time.RFC3339)))
	default:
	...
	total := len(entries)
	if total == 0 {
		cb.Note(e.i18n.T(MsgWsPickerEmpty))
		cb.Note(e.i18n.Tf(MsgWsPickerHint, e.baseDir))
		cb.Buttons(e.cardBackButton())
		return cb.Build(), nil
	}

	start, end, totalPages := workspacePickerPageBounds(page, total)
	if page < 1 {
		page = 1
	}
	if page > totalPages {
	...
	for i := start; i < end; i++ {
		entry := entries[i]
		btnText := e.i18n.T(MsgWsPickerBindBtn)
		btnType := "default"
		if entry.Path == currentWorkspace {
			btnText = e.i18n.T(MsgWsPickerCurrentBtn)
			btnType = "primary"
		}
		cb.ListItemBtn(
			...
			btnText,
			btnType,
			fmt.Sprintf("act:/workspace-bind %s|%d", url.QueryEscape(entry.Name), page),
		)
	}

	var navBtns []CardButton
	if page > 1 {
		navBtns = append(navBtns, e.cardPrevButton(fmt.Sprintf("nav:/workspace %d", page-1)))
	}
	navBtns = append(navBtns, e.cardBackButton())
	if page < totalPages {
		navBtns = append(navBtns, e.cardNextButton(fmt.Sprintf("nav:/workspace %d", page+1)))
	}
	cb.Buttons(navBtns...)
	if totalPages > 1 {
...
func (e *Engine) handleCardNav(action string, sessionKey string) *Card {
	...
	case "/workspace":
		...
		return e.renderWorkspacePickerCardSafe(sessionKey, page)
	case "/current":
	...
	case "/switch-id":
		return e.renderCurrentCard(sessionKey)
	case "/workspace-bind":
		_, page := parseWorkspaceBindActionArgs(args)
		return e.renderWorkspacePickerCardSafe(sessionKey, page)
	case "/delete-mode":
...
func (e *Engine) executeCardAction(cmd, args, sessionKey string) {
	...
	case "/workspace-bind":
		...
		workspaceName, _ := parseWorkspaceBindActionArgs(args)
		if workspaceName == "" {
			return
		}
		wsPath := filepath.Join(e.baseDir, workspaceName)
		info, err := os.Stat(wsPath)
		if err != nil {
			slog.Warn("workspace bind card action failed to stat workspace", "workspace", wsPath, "error", err)
			return
		}
		if !info.IsDir() {
			slog.Warn("workspace bind card action target is not a directory", "workspace", wsPath)
			return
		}
		channelName := ""
		if binding, _ := e.workspaceBindings.LookupEffective("project:"+e.name, channelKey); binding != nil {
			channelName = binding.ChannelName
		}
		e.workspaceBindings.Bind("project:"+e.name, channelKey, channelName, normalizeWorkspacePath(wsPath))
