...
func collectCardListActions(card *Card) []string {
	var actions []string
	for _, elem := range card.Elements {
		if item, ok := elem.(CardListItem); ok {
			actions = append(actions, item.BtnValue)
		}
	}
	return actions
}
...
func TestHandleCardNav_SwitchID_ReturnsCurrentCardWithoutListing(t *testing.T) {
	agent := &countingListAgent{}
	e := NewEngine("test", agent, []Platform{&stubPlatformEngine{n: "feishu"}}, "", LangEnglish)

	key := "feishu:chat:user"
	card := e.handleCardNav("act:/switch-id "+url.QueryEscape("sess-123")+"|"+url.QueryEscape("history summary")+"|2", key)
	if card == nil {
	...
	if !strings.Contains(text, "history summary") {
		t.Fatalf("card text = %q, want session summary", text)
	}
	if !strings.Contains(text, "sess-123") {
		t.Fatalf("card text = %q, want session id", text)
	}
}
...
func TestProcessInteractiveEvents_TouchesWorkspaceOnEvent(t *testing.T) {
	...
	e.processInteractiveEvents(state, session, e.sessions, "test:ch:user", "msg1", time.Now(), nil, nil, nil)
...
func TestWorkspace_CardPicker_DefaultCommand(t *testing.T) {
	p := &stubCardPlatform{stubPlatformEngine: stubPlatformEngine{n: "feishu"}}
	e := NewEngine("test", &stubAgent{}, []Platform{p}, "", LangEnglish)

	baseDir := t.TempDir()
	for _, name := range []string{"aiaudit", "cc-connect"} {
		if err := os.MkdirAll(filepath.Join(baseDir, name), 0o755); err != nil {
			t.Fatal(err)
		}
	}
	if err := os.WriteFile(filepath.Join(baseDir, "README.md"), []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.MkdirAll(filepath.Join(baseDir, ".hidden"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	msg := &Message{SessionKey: "feishu:ch1:user1", Content: "/workspace", ReplyCtx: "ctx"}
	e.handleCommand(p, msg, msg.Content)
	...
	card := p.repliedCards[0]
	if got := countCardActionValues(card, "act:/workspace-bind "); got != 2 {
		t.Fatalf("workspace bind actions = %d, want 2", got)
	}
	if card.Header == nil || card.Header.Title != e.i18n.T(MsgCardTitleWorkspace) {
		t.Fatalf("unexpected card title: %+v", card.Header)
	}
}

func TestWorkspace_CardPicker_BindWithoutArgs(t *testing.T) {
	p := &stubCardPlatform{stubPlatformEngine: stubPlatformEngine{n: "feishu"}}
	e := NewEngine("test", &stubAgent{}, []Platform{p}, "", LangEnglish)
	...
	msg := &Message{SessionKey: "feishu:ch1:user1", Content: "/workspace bind", ReplyCtx: "ctx"}
	e.handleCommand(p, msg, msg.Content)

	if len(p.repliedCards) != 1 {
		t.Fatalf("expected 1 replied card, got %d", len(p.repliedCards))
	}
	if got := countCardActionValues(p.repliedCards[0], "act:/workspace-bind "); got != 2 {
		t.Fatalf("workspace bind actions = %d, want 2", got)
	}
}

func TestWorkspace_CardBindAction_BindsAndRefreshesPicker(t *testing.T) {
	e := NewEngine("test", &stubAgent{}, nil, "", LangEnglish)

	baseDir := t.TempDir()
	wsDir := filepath.Join(baseDir, "aiaudit")
	if err := os.MkdirAll(wsDir, 0o755); err != nil {
		t.Fatal(err)
	}
	bindStore := filepath.Join(t.TempDir(), "bindings.json")
	...
	sessionKey := "feishu:ch1:user1"
	card := e.handleCardNav("act:/workspace-bind "+url.QueryEscape("aiaudit")+"|1", sessionKey)

	channelKey := workspaceChannelKey("feishu", "ch1")
	normalizedWsDir := normalizeWorkspacePath(wsDir)
	if got := e.workspaceBindings.Lookup("project:test", channelKey); got == nil || got.Workspace != normalizedWsDir {
		t.Fatalf("expected binding %q, got %+v", normalizedWsDir, got)
	}
	if card == nil {
		t.Fatal("expected refreshed workspace picker card")
	}
	btn, ok := findCardAction(card, "act:/workspace-bind "+url.QueryEscape("aiaudit")+"|1")
	if !ok {
		t.Fatal("expected current workspace row action")
	}
	if btn.Type != "primary" || btn.Text != e.i18n.T(MsgWsPickerCurrentBtn) {
		t.Fatalf("unexpected current workspace button: %+v", btn)
	}
}

func TestWorkspace_CardPicker_SortsByLatestActivity(t *testing.T) {
	p := &stubCardPlatform{stubPlatformEngine: stubPlatformEngine{n: "feishu"}}
	e := NewEngine("test", &stubAgent{}, []Platform{p}, "", LangEnglish)

	baseDir := t.TempDir()
	oldDir := filepath.Join(baseDir, "old-project")
	midDir := filepath.Join(baseDir, "mid-project")
	hotDir := filepath.Join(baseDir, "hot-project")
	for _, dir := range []string{oldDir, midDir, hotDir} {
		if err := os.MkdirAll(dir, 0o755); err != nil {
			t.Fatal(err)
		}
	}
	now := time.Now()
	if err := os.Chtimes(oldDir, now.Add(-3*time.Hour), now.Add(-3*time.Hour)); err != nil {
		t.Fatal(err)
	}
	if err := os.Chtimes(midDir, now.Add(-2*time.Hour), now.Add(-2*time.Hour)); err != nil {
		t.Fatal(err)
	}
	if err := os.Chtimes(hotDir, now.Add(-4*time.Hour), now.Add(-4*time.Hour)); err != nil {
		t.Fatal(err)
	}

	bindStore := filepath.Join(t.TempDir(), "bindings.json")
	e.SetMultiWorkspace(baseDir, bindStore)
	e.workspacePool.GetOrCreate(normalizeWorkspacePath(hotDir)).Touch()
	...
	actions := collectCardListActions(p.repliedCards[0])
	if len(actions) < 3 {
		t.Fatalf("expected at least 3 workspace rows, got %d", len(actions))
	}
	wantFirst := "act:/workspace-bind " + url.QueryEscape("hot-project") + "|1"
	if actions[0] != wantFirst {
		t.Fatalf("first action = %q, want %q", actions[0], wantFirst)
	}
}

func TestWorkspace_CardPicker_FirstPageTopTenAndSecondPageRemainder(t *testing.T) {
	p := &stubCardPlatform{stubPlatformEngine: stubPlatformEngine{n: "feishu"}}
	e := NewEngine("test", &stubAgent{}, []Platform{p}, "", LangEnglish)
	...
	for i := 1; i <= 12; i++ {
		...
		modTime := now.Add(-time.Duration(12-i) * time.Minute)
	...
	firstPage := p.repliedCards[0]
	if got := countCardActionValues(firstPage, "act:/workspace-bind "); got != 10 {
		t.Fatalf("first page workspace rows = %d, want 10", got)
	}
	if _, ok := findCardAction(firstPage, "nav:/workspace 2"); !ok {
		t.Fatal("expected next-page action on first page")
	}

	secondPage := e.handleCardNav("nav:/workspace 2", "feishu:ch1:user1")
	if secondPage == nil {
		t.Fatal("expected second page card")
	}
	if got := countCardActionValues(secondPage, "act:/workspace-bind "); got != 2 {
		t.Fatalf("second page workspace rows = %d, want 2", got)
	}
}
